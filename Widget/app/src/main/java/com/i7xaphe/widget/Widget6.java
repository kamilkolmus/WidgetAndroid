...
import static com.i7xaphe.widget.Widget6.switchAction.SHOW;

public class Widget6 extends Service implements View.OnTouchListener {
    ...
    public void onCreate() {
        ...
        if (sheredpreferences.getBoolean("widgetBoundaries", MySettings.boundariesOn)) {
            tlparams = new WindowManager.LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT, LayoutParams.TYPE_PHONE,
                    WindowManager.LayoutParams.FLAG_NOT_FOCUSABLE| LayoutParams.FLAG_HARDWARE_ACCELERATED, PixelFormat.TRANSPARENT);
        } else {
            tlparams = new WindowManager.LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT, LayoutParams.TYPE_PHONE,
                    WindowManager.LayoutParams.FLAG_NOT_FOCUSABLE |LayoutParams.FLAG_HARDWARE_ACCELERATED |LayoutParams.FLAG_LAYOUT_NO_LIMITS, PixelFormat.TRANSPARENT);
        }
        ...
        linearLayout = (LinearLayout) layoutInflater.inflate(R.layout.widget_6, null);
    ...
    //    gifViewWidget.setDrawingCacheEnabled(true);
        ...
        switch (color) {
         ...
         marginWidgetTopBot = (int) (radius * 0.866 * scaleY - iconSize / 2 - widgetSize / 2);
        ...
        if (isFileGif) {
            ...
            marginWidgetTopBot = (int) (radius * 0.866 * scaleY - iconSize / 2 - newMovieY / 2);
        ...
        spaceBOT.getLayoutParams().width = marginIconLeftRight;
        spaceTOP.getLayoutParams().width = marginIconLeftRight;
        ...
        if (sheredpreferences.getBoolean("rotateAnim", MySettings.randomAnim)) {
            ...
            anim_when_show_icons = AnimationUtils.loadAnimation(getApplicationContext(), R.anim.widget_when_hide_icon);
        }



        show = new ArrayList<>();
        show.add(AnimationUtils.loadAnimation(getApplicationContext(), R.anim.show));
        show.add(AnimationUtils.loadAnimation(getApplicationContext(), R.anim.show));
        show.add(AnimationUtils.loadAnimation(getApplicationContext(), R.anim.show));
        show.add(AnimationUtils.loadAnimation(getApplicationContext(), R.anim.show));
        show.add(AnimationUtils.loadAnimation(getApplicationContext(), R.anim.show));
...
//                Log.i("LOOP1", ""+manager.getNameForUid(taskInfos.get(i).uid));
//                if(packageName.equals(manager.getNameForUid(taskInfos.get(i).uid))){
//                    Log.i("LOOP1", ""+manager.getNameForUid(taskInfos.get(i).uid));
//                    List<ActivityManager.AppTask> taskInfos2= activityManager.getAppTasks();
//                    for(int k=0;k<taskInfos2.size();k++){
//                        int id=taskInfos2.get(k).getTaskInfo().id;
//                        Log.i("Loop2","1"+taskInfos2.get(k).getTaskInfo().id);
    ...
    public boolean onTouch(View v, MotionEvent event) {
            ...
            switch (event.getAction()) {
                case MotionEvent.ACTION_DOWN:
                    ...
                    x = tlparams.x;
                    y = tlparams.y;
                    //  Log.v("ACTION_DOWN xy",x+" "+y);

                    pressedX = event.getRawX();
                    pressedY = event.getRawY();
                    // Log.v("ACTION_DOWN pressed",pressedX+" "+pressedY);
...
//            getBaseContext().startActivity(showTaskIntent);
        ...
        for (int i = 0; i <6; i++) {
            ...
            imageViewList.get(i).postOnAnimationDelayed(new Runnable() {
                ...
                public void run() {
                    ...
                    imageViewList.get(finalI).startAnimation(show.get(finalI));
